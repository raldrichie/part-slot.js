function EZSlots(id,useroptions){
	...
	this.init = function(){
		...
		for(var i = 0; i < this.reelCount; i++){
			...
			var window = this.jqo[0].children[i];
			window.append(jqoSlider[0]);
			/**var jqoWindow = $('<div class="window window_"'+i+'></div>');
			this.scaleSlider(jqoWindow).append(jqoSlider); //make window right size and put slider in it
	...
	this.scaleJqo = function(jqo){
		...
		this.scaleSlider = function(jqo){
		jqo.css("height",this.sliderHeight+"px").css("width",this.sliderWidth+"px");
		return jqo;
	}
	...
	this.spinOne = function(jqoSlider,whichReel,shouldWin){
		...
		//marginTop *= (that.sliderHeight / that.height) - 1.0;
		marginTop += 100;	
	...
	return {
		...
		customSpin : function(resultSet){
			that.winningSet = resultSet;
			return that.spinAll(true);
		}
	}
}
